...
const g=makeSurvivalShell(SurvivalGame,'urban');g.start(undefined,undefined,{mapId:'urban'});
assert.equal(g.map,'urban');assert.equal(g.enemies.length,50);assert.equal(g.state.remainingAlive,51);assert.equal(g.loot.length,300);assert.equal(g.layout.compounds.length,36);assert.equal(g.layout.interiorCover.length,36);assert.equal(g.layout.routeCover.length,150);assert.equal(g.state.enemyMarkers.length,0);
...
for(const b of g.layout.compounds){
 ...
 assert(!g.line(new T.Vector3(b.x-b.w/2-5,y+b.h+2,b.z),new T.Vector3(b.x+b.w/2+5,y+b.h+2,b.z)),`${b.id} upper volume leaks beams`);
...
for(const [i,[x,z]] of g.layout.spawns.entries()){
 ...
 assert(Math.hypot(x,z)<survivalZone(0,'urban').radius,`spawn ${i} outside initial zone`);
...
console.log(JSON.stringify({map:'urban',competitors:51,buildings:36,portals,connectedLoot:g.loot.length,interiorLoot:indoors,categories,wptCount,blocks:g.blocks.length,navConnectedNodes:queue.length,urbanDetails:g.urbanDetails}));
...
g.start(undefined,undefined,{mapId:'urban'});const originalRandom=Math.random;Math.random=seeded(38741);let ticks=0;
...
assert.equal(g.mode,'result','urban match never terminates');assert.equal(g.result.gameMode,'survival');assert.equal(g.result.mapId,'urban');assert(Object.isFrozen(g.result));assert(g.result.placement>=1&&g.result.placement<=51);g.emit();assert.deepEqual(g.state.enemyMarkers,[]);const result=g.result;g.update(10);assert.equal(g.result,result);
